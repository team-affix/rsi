...
struct RecursorApplicatorTest : public ::testing::Test {
    ...
    std::shared_ptr<expr> index = std::make_shared<expr>(expr{expr::var{3}});
    std::shared_ptr<expr> rand = std::make_shared<expr>(expr{expr::var{4}});
    ...
    std::shared_ptr<expr> with_index = std::make_shared<expr>(expr{expr::app{with_y, index}});
    std::shared_ptr<expr> result = std::make_shared<expr>(expr{expr::app{with_index, rand}});
};
...
TEST_F(RecursorApplicatorTest, ApplyIsLeftAssociated) {
    ...
    EXPECT_CALL(make_app, make_app(with_y, index)).WillOnce(Return(with_index));
    EXPECT_CALL(make_app, make_app(with_index, rand)).WillOnce(Return(result));
    EXPECT_EQ(applicator.apply(rec, flag, y, index, rand), result);
}
